...
def period_format(input_txt, str_thresh):
    new_text = ""
    format_list = []
    text_file_list = []
    str_num = 0
...
def concat_textfiles(file_names):
    with open("summary/all_result.txt", mode="w", encoding="utf-8") as new_file:
        for name in file_names:
            print(f"テキストファイル結合中・・・{name}")
            with open(f"summary/{name}") as f:
                for line in f:
                    new_file.write(line)
            new_file.write("\n")
